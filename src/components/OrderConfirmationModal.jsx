                    ...
                    <ul className="summary-list">
                        {cart.map(item => (
                            <li key={item.id} className="summary-item">
                                <img className="img-conf" src={getImage(item.image.thumbnail)} alt={item.name} />
                                <div className="cart-info">
                                    <p className="name-confirmation">{item.name}</p>
                                    <div className="qty-confirmation">
                                        <p className="item-qty" >{item.quantity}x</p>
                                        <p className="item-price">@ ${item.price.toFixed(2)}</p>
                                    </div>
                                </div>
                                <p className="final-total">${(item.quantity * item.price).toFixed(2)}</p>
                            </li>
                        ))}
